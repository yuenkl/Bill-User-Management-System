...
- Define a fakeable `UserRemoteDataSource` with initial/next-page, create, and delete operations. Its responses never escape directly to UI/ViewModel state.
...
- Process mutations FIFO, then refresh the initial `/users` snapshot; subsequent pages append through the serialized pagination path.
