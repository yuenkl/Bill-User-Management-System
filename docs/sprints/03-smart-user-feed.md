...
- Fetch `/users` without pagination parameters to read `X-Pagination-Pages`, then display that final page and follow `X-Links-Previous` for each earlier page when the feed reaches its end.
- Record response order as `serverPosition` and use the current injected clock only for newly observed rows.
- Map I/O, timeout, 401/403, 422, 429, 5xx, serialization, malformed pagination-page-header, and malformed previous-page-link failures into typed data/domain errors.
...
- Pagination pages: final-page discovery, ordered previous pages, invalid headers or previous links, empty pages, retry, and end-of-list behavior.
...
- Online cold start displays the current final page and scrolling appends `page=295`, `page=294`, and earlier pages from GoRest.
